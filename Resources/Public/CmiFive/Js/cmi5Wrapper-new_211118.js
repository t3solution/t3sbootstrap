...
document.addEventListener(
  "DOMContentLoaded", () => {
    ...
    if (!document.querySelectorAll(".course-login")) sessionStorage.setItem("courseLoggedIn", 1);
...
function feLogIn() {
  ...
  let formData = document.querySelectorAll(".course-login form")[0],
    ...
    coursePw = location.pathname + document.querySelectorAll(".auth")[0].innerHTML.trim(),
    courseId = location.pathname;
  document.querySelectorAll(".auth")[0].remove();
...
function customizeTemplate() {
  ...
  if (!sessionStorage.getItem("startPageId") && parseInt(sessionStorage.getItem("courseLoggedIn")) > 0) sessionStorage.setItem("startPageId", pageId);
  ...
  if (pageItems.length > 0) {
    ...
    pageItemsA[0].appendChild(pagArrow[0]);
  ...
  if (sessionStorage.getItem("startPageId") == pageId) {
