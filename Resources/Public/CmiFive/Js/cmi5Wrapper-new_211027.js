...
document.addEventListener(
  "DOMContentLoaded", () => {
    ...
    else if (location.href.indexOf("?") == -1 && parseInt(sessionStorage.getItem("courseLoggedIn")) > 0) {
    ...
    if (location.href.indexOf("?") != -1 && location.href.indexOf("simUser") == -1) {
...
function parse(val) {
  ...
  //if (result == result_) sessionStorage.setItem("cmi5No", "true");
  return result;
}
...
document.onreadystatechange = function() {
  ...
  if ('complete' === document.readyState && typeof H5P !== 'undefined' && H5P.externalDispatcher && cmi5Controller) H5P.externalDispatcher.on('xAPI', handleH5P);
};